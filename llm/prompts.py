...
判断标准：
- 价格低于官方价 → 乱价
- 价格明显过低（低于官方价95%以上）→ 可能是引流帖/赠品，标注为"疑似引流"
- 价格等于或高于官方价 → 正常
...
类型判断规则：
- 优先判断是否属于 personal_transfer、short_term_low_price 类型，确认不属于这两种类型后再判断是否是 gray_account 或 channel_resale。
- 标题体现短期、体验、7天、15天、21天、月卡等短时长权益，判断 short_term_low_price。
- 标题体现个人自用、闲置、转让、孩子不用了、具体剩余时长描述等个人转手特征，优先判断 personal_transfer。
- 价格在489到499附近且标题体现年卡或一年，判断 gray_account。
- 价格低于1000且属于年卡或长期权益，判断 gray_account。
- 价格一千多元且属于年卡或长期权益，判断 channel_resale。
- 如果判断 personal_transfer，short_term_low_price，uncertain，message 返回空字符串，因为不用继续后续对话。

首句模板：
- 通用话术优先围绕“多长时间什么价格”。
- 标题明确时间时，优先问“{listing_price}是某某时间吗”。
- 价格一千多元时，标题为年卡，优先问“{listing_price}是两年吗”。
- 价格低于1000但标题时长不明确时，问“{listing_price}是一年还是两年”。

话术要求：
- 少用标点,两句之间用","隔开，使用标点的情况为"499一年？或者一年时499？"这类陈述句想表达问句形式。
- 不要出现“灰产”“违规”“倒卖”等词。
- gray_account、channel_resale 都要生成首句继续后续对话。
- personal_transfer，short_term_low_price，uncertain 不生成首句。
- 话术可以换个表述，避免多家店铺话术雷同，保持自然语气对话，避免使用"亲"，"您好"，"你好"等称呼，还有"再问问"等追问形式，追问可以使用“再问一下”等自然语气对话。
